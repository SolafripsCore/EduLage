...
export function ProgrammeCard({ programme, priority = false, discovery = false }: { programme: Programme; priority?: boolean; discovery?: boolean }) {
  ...
  return <Link href={`/programmes/${programme.slug}`} className="group card-hover flex h-full flex-col overflow-hidden rounded-xl border border-line bg-white focus-visible:ring-2 focus-visible:ring-teal-500 focus-visible:ring-offset-2">
    ...
    <div className="flex flex-1 flex-col p-5 pt-8">
      ...
      {discovery && <div className="mt-3 flex min-h-7 flex-wrap gap-1.5"><span className="rounded-full bg-teal-500/10 px-2.5 py-1 text-[10px] font-bold text-teal-700">{programme.deliveryMode === "Fully online" ? "Fully online" : "OEC support available"}</span>{programme.studyMode === "Part-time" && <span className="rounded-full bg-surface px-2.5 py-1 text-[10px] font-bold text-ink-600">Flexible schedule</span>}</div>}
      <div className={`${discovery ? "mt-3" : "mt-4"} space-y-2 text-xs text-ink-600`}>
        {!discovery && <p className="flex items-center gap-2"><MapPin size={14} className="shrink-0 text-ink-400" />{institution.city}, {institution.country}</p>}
        ...
        {!discovery && <p className="flex items-center gap-2"><Monitor size={14} className="shrink-0 text-ink-400" />{programme.deliveryMode}</p>}
