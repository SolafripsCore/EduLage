...
export function ProgrammeCard({
  ...
  return (
    <Link
      ...
      className="group card-hover flex h-full flex-col overflow-hidden rounded-xl border border-line bg-white focus-visible:ring-2 focus-visible:ring-teal-500 focus-visible:ring-offset-2"
    >
      ...
      <div className="flex flex-1 flex-col p-5 pt-8">
        ...
        <h3 className="line-clamp-2 mt-2 min-h-12 text-[17px] font-semibold leading-6 text-navy-800 transition-colors duration-200 group-hover:text-teal-600">
          {programme.title}
        </h3>
        {discovery && (
          <div className="mt-3 flex min-h-7 flex-wrap gap-1.5">
            ...
            <span className="rounded-full bg-teal-500/10 px-2.5 py-1 text-xs font-bold text-teal-700">
              {programme.deliveryMode === "Fully online"
                ? "Fully online"
                : "OEC support available"}
            </span>
            {programme.studyMode === "Part-time" && (
              <span className="rounded-full bg-surface px-2.5 py-1 text-xs font-bold text-ink-600">
                Flexible schedule
              </span>
            )}
          </div>
        ...
        <div
          className={`${discovery ? "mt-3" : "mt-4"} space-y-2 text-xs text-ink-600`}
        >
          {!discovery && (
          ...
          <p className="flex items-center gap-2">
            <Clock3 size={14} className="shrink-0 text-ink-400" />
            {programme.durationMonths} months <span aria-hidden>·</span>{" "}
            {programme.studyMode}
          </p>
          {!discovery && (
            <p className="flex items-center gap-2">
          ...
          <p className="flex items-center gap-2">
            ...
            Next intake: {programme.nextIntake}
          </p>
        </div>
        <div className="mt-auto flex items-end justify-between gap-3 border-t border-line pt-4">
          <div>
            <p className="text-xs text-ink-400">Tuition from</p>
            <p className="mt-1 text-sm font-semibold text-navy-800">
              {tuition} / {programme.tuitionPeriod}
            </p>
          </div>
          <span className="arrow-slide flex items-center gap-1.5 text-xs font-semibold text-navy-800 group-hover:text-teal-600">
