...
export function Header() {
  ...
  return <><div className="bg-teal-600 py-2 text-center text-[11px] font-semibold tracking-wide text-white">DEVELOPMENT ENVIRONMENT — CATALOGUE CONTENT IS ILLUSTRATIVE UNTIL INSTITUTIONAL PARTICIPATION IS CONFIRMED</div><div className="hidden h-9 bg-navy-900 text-xs text-white/70 lg:block"><div className="container-page flex h-full items-center justify-between"><span>The Global Education Village</span><div className="flex items-center divide-x divide-white/20"><>{utilityLinks.map(([label, href], index) => <Link key={href} href={href} className={`${index ? "pl-4" : ""} pr-4 last:pr-0 hover:text-white focus-visible:rounded-sm focus-visible:ring-2 focus-visible:ring-teal-400`}>{label}</Link>)}</></div></div></div><header ref={headerRef} className="sticky top-0 z-40 border-b border-line bg-white/95 backdrop-blur-md">
