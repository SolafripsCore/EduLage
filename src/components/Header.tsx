...
export function Header() {
  ...
  return <header className="sticky top-0 z-40 border-b border-line bg-white">
    <div className="container-page flex h-[72px] items-center justify-between gap-5">
      <Link href="/" aria-label="Edulage home" className="shrink-0"><Image src="/brand/edulage-logo.png" alt="Edulage" width={110} height={70} className="h-14 w-[110px] object-contain" priority /></Link>
    ...
    {open && <div className="fixed inset-0 z-50 bg-white p-6 md:hidden">
      <div className="flex items-center justify-between"><Link href="/" onClick={() => setOpen(false)}><Image src="/brand/edulage-logo.png" alt="Edulage" width={110} height={70} className="h-14 w-[110px] object-contain" /></Link><button onClick={() => setOpen(false)} aria-label="Close navigation" className="rounded-md p-2 text-navy-800"><X size={25} /></button></div>
