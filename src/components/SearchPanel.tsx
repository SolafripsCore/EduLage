...
export function SearchPanel() {
  ...
  return <div>
    <form action="/programmes" className="grid gap-2 rounded-lg bg-white p-3 shadow-lg md:grid-cols-[2fr_1fr_1fr_1fr_auto]">
      ...
      <label className="sr-only" htmlFor="discipline-search">Discipline</label><select id="discipline-search" name="discipline" className="min-h-12 rounded-md border border-line bg-white px-3 text-sm text-ink-600"><option value="">Discipline</option><option>Business &amp; Management</option><option>Computer Science &amp; IT</option><option>Data &amp; AI</option><option>Engineering</option><option>Health &amp; Medical Sciences</option><option>Education</option><option>Law</option><option>Public Administration</option><option>Social Sciences</option><option>Natural Sciences</option><option>Environment &amp; Sustainability</option><option>Agriculture &amp; Food Security</option><option>Arts &amp; Humanities</option></select>
      <label className="sr-only" htmlFor="country-search">Country</label><select id="country-search" name="country" className="min-h-12 rounded-md border border-line bg-white px-3 text-sm text-ink-600"><option value="">Country</option>{countries.map((country) => <option key={country}>{country}</option>)}</select>
